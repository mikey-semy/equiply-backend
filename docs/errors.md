...
- Запрашиваемая страница не существует (PageNotFoundError)
...
Примеры использования:
- Клиент не завершил запрос в отведенное время (RequestTimeoutError)

### 409 Conflict
Описание: Конфликт с текущим состоянием ресурса
Примеры использования:
- Попытка создать пользователя с уже существующим email (EmailAlreadyExistsError)
- Попытка обновить устаревшую версию ресурса (ResourceOutOfDateError)

### 410 Gone
Описание: Ресурс больше не доступен
Примеры использования:
- Доступ к удаленному контенту (GoneError)
- Устаревший API-эндпоинт (DeprecatedEndpointError)

### 413 Payload Too Large
Описание: Размер запроса превышает допустимый лимит
Примеры использования:
- Загрузка слишком большого файла (FileTooLargeError)
- Слишком длинный запрос (RequestTooLongError)

### 415 Unsupported Media Type
Описание: Формат запроса не поддерживается
Примеры использования:
- Отправка XML, когда сервер принимает только JSON (UnsupportedMediaTypeError)
- Загрузка файла неподдерживаемого формата (UnsupportedFileFormatError)

### 422 Unprocessable Entity
Описание: Запрос синтаксически корректен, но содержит семантические ошибки
Примеры использования:
- Недействительный токен (TokenInvalidError)
- Валидация данных не прошла (ValidationError)
- Логические ошибки в запросе (LogicError)
...
Примеры использования:
- Метод API находится в разработке (NotImplementedError)
- Функция еще не реализована (NotImplementedError)

### 502 Bad Gateway
Описание: Ошибка шлюза или прокси
Примеры использования:
- Ошибка при обращении к внешнему сервису (ExternalServiceError)
- Проблемы с промежуточным сервером (ProxyError)

### 503 Service Unavailable
Описание: Сервер временно недоступен
Примеры использования:
- Сервер перегружен (ServerOverloadedError)
- Техническое обслуживание (MaintenanceError)
- Временная недоступность сервиса (ServiceUnavailableError)

### 504 Gateway Timeout
Описание: Время ожидания ответа от вышестоящего сервера истекло
Примеры использования:
- Внешний API не ответил вовремя (ExternalServiceTimeoutError)
- Проблемы с сетью (NetworkTimeoutError)
