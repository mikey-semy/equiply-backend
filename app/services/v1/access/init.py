...
class PolicyInitService:
    ...
    async def initialize_default_policies(self, policies_dir: Path) -> int:
        ...
        if existing_policies:
            logger.info(f"Базовые политики уже существуют ({len(existing_policies)} шт.)")
            return 0
        ...
        for policy_file in policies_dir.glob("*.json"):
            ...
            try:
                ...
                for policy in policies:
                    ...
                    created_policy = await self.access_service.create_default_policy(policy)
                    logger.info(f"Создана базовая политика: {created_policy.name} для {resource_type}")
                    total_created += 1

            except Exception as e:
                logger.error(f"Ошибка при загрузке политик из файла {policy_file}: {str(e)}")
    ...
    async def apply_default_policies_to_workspace(
        ...
        for default_policy in default_policies:
            try:
                ...
                if (default_policy.resource_type == "workspace" and
