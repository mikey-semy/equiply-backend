...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(
            ["column_id"], ["kanban_columns.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(
            ["integration_id"], ["module_integrations.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["kanban_card_id"], ["kanban_cards.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["list_item_id"], ["list_items.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["post_id"], ["posts.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(
            ["table_row_id"], ["table_rows.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.add_column(
        "list_definitions", sa.Column("template_id", sa.Integer(), nullable=True)
    )
    ...
    op.add_column(
        "module_templates", sa.Column("template_data", sa.JSON(), nullable=False)
    )

    # Используем прямой SQL для изменения типа колонки с USING-выражением
    op.execute(
        "ALTER TABLE module_templates ALTER COLUMN module_type TYPE moduletype USING module_type::moduletype"
    )

    op.drop_column("module_templates", "schema")
    op.add_column(
        "table_definitions", sa.Column("template_id", sa.Integer(), nullable=True)
    )
    op.create_foreign_key(
        None,
        "table_definitions",
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE module_templates ALTER COLUMN module_type TYPE VARCHAR(50) USING module_type::text"
    )
